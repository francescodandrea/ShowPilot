...
async function triggersupd(data, videorefresh=true){
    ...
    if(data.meta.syncedmedia){
        ...
        document.querySelector("#vid").src="/videostream?file="+data.meta.syncedmedia[0].filename;
        ...
        data.meta.syncedmedia.forEach(media => {
            ...
            btn.dataset.full=media.filename;
...
function se_videochoice(but){
    ...
    document.querySelector("#vid").src="/videostream?file="+but.dataset.full; //temp clouding solution
...
if(i!=lbuffer){
    ...
    let linecont=document.querySelector("#live > div.lines");

    if(linecont.childElementCount<5 || Math.abs(lbuffer-i)>1){
        linecont.innerHTML="";
        for (let j=0; j<15; j++) {
        ...
        if(i>lbuffer){
            linecont.appendChild(div);
            linecont.removeChild(linecont.firstElementChild);
        } else {
            linecont.prepend(div);
            linecont.removeChild(linecont.lastElementChild);
        }
    }
    
    lbuffer=i;
    console.log("LINE "+i);
    document.querySelector("#linesrccount").innerHTML="Line "+i;
    countengine(i);
}
}
...
let linesrcresult=null;
let linesrccurr=null;
let linesrcshow=null;

function linesrc(val,down){
    if(linesrccurr!=val && val!=null && (val!="" && down==undefined)){
        linesrccurr=val;
        console.log(val)
        linesrcresult=[];
        liness.forEach( (line,i) => {
            if(line[0].match(val)!=null) linesrcresult.push(i);
        });
        console.log(linesrcresult)

        if(linesrcresult.length>0){
            document.querySelector("#linesrccount").innerHTML="1 of "+linesrcresult.length;
            toline(linesrcresult[0]);
            linesrcshow=0;
        } else document.querySelector("#linesrccount").innerHTML="no result";
        
    } else {
        if(down==true && linesrcresult.length>linesrcshow+1){
            linesrcshow++;
            toline(linesrcresult[linesrcshow]);
            document.querySelector("#linesrccount").innerHTML=(linesrcshow+1)+" of "+linesrcresult.length;
        }
        if(down==false && linesrcshow>0){
            linesrcshow--;
            toline(linesrcresult[linesrcshow]);
            document.querySelector("#linesrccount").innerHTML=(linesrcshow+1)+" of "+linesrcresult.length;
        }
...
function countengine(i){
    nomi.forEach((nome,j) => {
        ...
        do{
            ...
            if(step>liness.length-10) check=false;
        } while (check)
